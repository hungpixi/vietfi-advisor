...
| **Database** | Supabase (setup sẵn, chưa kết nối) | Đang dùng localStorage tạm |
| **Scraping** | Cheerio (installed) | Chờ Hoàng implement crawl |
| **Deploy** | Vercel (target) | Chưa deploy |
...
│   │   │   └── vet-vang/               # (reserved) Vẹt Vàng API
│   │   ├── dashboard/
...
### 🔴 Phase 1: Database & Auth (ưu tiên số 1)
- [ ] Tạo Supabase project → schema (users, expenses, debts, portfolios)
...
1. **Supabase Auth + Database** — data persist, multi-device sync
2. **Real-time Market Data** — crawl tự động mỗi 15 phút
3. **Push Notifications** — nhắc nhở ghi chi tiêu
4. **Voice Clone** — giọng Vẹt Vàng clone từ ZinZin (VieNeu-TTS)
5. **Mascot Animation** — Rive/Spine animation cho Vẹt Vàng
6. **Mobile App** — React Native hoặc PWA
