...
export function PayDebtModal({ debt, onClose, onPay }: { debt: UIDebt; onClose: () => void; onPay: (id: string, amount: number) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-sm glass-card p-6 border-t border-[#22C55E]/20 relative overflow-hidden">
