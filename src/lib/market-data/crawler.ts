...
type CacheableRequestInit = RequestInit & {
  next?: { revalidate?: number }
}

async function fetchWithCache(url: string, options: CacheableRequestInit = {}, timeoutMs = 8000) {
...
async function generateAiSummary(snapshot: MarketSnapshot): Promise<string | null> {
  if (process.env.DISABLE_GEMINI_SUMMARY === '1' || !getGeminiApiKey()) {
    return null
  }
